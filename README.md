...
- **`codegraph audit` — MEASURED precision, not claimed**: samples tree-sitter edges, verifies them against the compiler oracle merged into your graph (SCIP/IndexStore), and stores per-tier precision that MCP `stats` and `report` quote back to the agent. Published, reproducible numbers ([docs/BENCHMARK.md](docs/BENCHMARK.md)): **98.7% on zod (TS oracle, ImportNarrowed 62/62 = 100%) · 93.8% on fastapi (Python oracle) · 95.0% on a 55k-node private monorepo (IndexStore)** — lower bounds, seeded sampling. The audit even catches our own bugs: a 27%-precision fallback tier it exposed was deleted, not defended.
...
**MCP (19 tools):** `search`, `callers`, `callees`, `blast_radius`, `trace_path`, `context`, `changes`, `dead_code`, `co_changes`, `implementers`, `routes`, `important`, `semantic_search`, `flows`, `graph_query`, `get_node` (with exact-span `snippet`), `architecture`, `stats`, `dead_code_v2` — each with agent-guidance descriptions, coverage signals, and `_hints`.

**The layered-answer contract:** every graph answer returns a **precise layer** (resolved edges — never guessed) plus a **labeled textual layer** (parser-verified call tokens the resolver dropped, evidence-filtered: local-binding shadowing, external-import exclusion, nearest-definition attribution) plus coverage and a `_fallback`. Measured on the public eval: recall 0.98 at 319 bytes/answer (grep: 0.94 at 2,701) with precision 0.66 — truth and coverage, separately labeled, never blended. Search is **definition-first**: bm25 + label/exact-name ranking puts the actual definition above the twelfth test that mentions it.
...
crates/
  ...
  codegraph-mcp        MCP server (19 tools, generation-keyed graph cache, fs-watcher, coverage signals)
