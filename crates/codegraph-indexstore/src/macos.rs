...
extern "C" {
    ...
    fn indexstore_store_units_apply_f(
        s: Store,
        sorted: c_uint,
        ctx: *mut c_void,
        f: StrApplier,
    ) -> bool;
    fn indexstore_unit_reader_create(
        s: Store,
        name: *const c_char,
        error: *mut ErrorT,
    ) -> UnitReader;
    ...
    fn indexstore_unit_reader_dependencies_apply_f(
        r: UnitReader,
        ctx: *mut c_void,
        f: DepApplier,
    ) -> bool;
    ...
    fn indexstore_record_reader_create(
        s: Store,
        name: *const c_char,
        error: *mut ErrorT,
    ) -> RecordReader;
    fn indexstore_record_reader_dispose(r: RecordReader);
    fn indexstore_record_reader_occurrences_apply_f(
        r: RecordReader,
        ctx: *mut c_void,
        f: OccApplier,
    ) -> bool;
    ...
    fn indexstore_occurrence_relations_apply_f(
        o: Occurrence,
        ctx: *mut c_void,
        f: RelApplier,
    ) -> bool;
...
extern "C" fn rel_cb(ctx: *mut c_void, rel: SymbolRelation) -> bool {
    unsafe {
        if indexstore_symbol_relation_get_roles(rel) & ROLE_REL_CALLEDBY != 0 {
            let usr = decode(indexstore_symbol_get_usr(
                indexstore_symbol_relation_get_symbol(rel),
            ));
            if !usr.is_empty() {
...
extern "C" fn occ_cb(ctx: *mut c_void, occ: Occurrence) -> bool {
    unsafe {
        ...
        let usr = decode(indexstore_symbol_get_usr(indexstore_occurrence_get_symbol(
            occ,
        )));
        if usr.is_empty() {
            return true;
...
pub fn read_occurrences(store_path: &Path, root: &Path) -> anyhow::Result<Vec<Occ>> {
    ...
    if store.is_null() {
        let msg = if err.is_null() {
            ...
            unsafe {
                CStr::from_ptr(indexstore_error_get_description(err))
                    .to_string_lossy()
                    .into_owned()
            }
        };
    ...
    unsafe {
        indexstore_store_units_apply_f(store, 0, &mut units as *mut _ as *mut c_void, unit_cb)
    };

    let mut occs: Vec<Occ> = Vec::new();
    for unit_name in &units {
        let Ok(cu) = CString::new(unit_name.as_str()) else {
            continue;
        };
        ...
        for (rname, rfile) in &records {
            ...
            let Ok(cr) = CString::new(rname.as_str()) else {
                continue;
            };
